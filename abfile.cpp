...
//	Comp cmp;
	...
	while (blocks.size() > maxblocks) { //remove last and write to disk.
	...
	while (len>0)
	{
		...
		int read=b->read();
                if (offset+read<mLength)
                {
                    ...
                    read=b->dataLen=std::min<int>(BLOCKSIZE, mLength-BLOCKSTART(b));
                }
		int off = offset - BLOCKSTART(b);
...
int ABFile::getattr(cstr path, struct stat *st, bool readsize /* = false */)
{
	...
	if (r==0)
	{//found meta file, this is a file!
            if (!readsize)
                return 0;
